...
class MolLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, base_path, radius=12, batch_size=32, receptors=None,
                 **kwargs):
        ...
        for epoch in range(epochs):
            ...
            for self.batch, ((p, v, m), y_true) in enumerate(self.data_loader):
                ...
                print_with_overwrite(
                    ...
                    'Time remaining:', eta,
                    '\nActive: {0:.4f}'.format(active_mean_pred),
                    'Decoy: {0:.4f}'.format(decoy_mean_pred)
                )
...
network = models.GninaNet(
    22, ds_frac=1., num_outputs=1, k=[32, 64, 128], nbhd=20, act='relu',
    bn=True, num_layers=2, mean=True, pool=True, liftsamples=1,
    fill=0.1, group=SE3(), knn=False, cache=False)

network = lieConv.LieResNet(
